...
impl Inode {
    ...
    pub fn root_inode(efs: &Arc<dyn VFS>) -> Arc<Self> {
        let efs_concrete = match efs.clone().downcast_arc::<EasyFileSystem>() {
            Ok(fs) => fs,
            Err(_) => panic!("Failed to downcast Arc<dyn VFS> to Arc<EasyFileSystem>"),
        };        
...
impl InodeTrait for Inode {
    ...
    fn from_ent(
        ...
        let parent_dir_specific = match parent_dir.clone().downcast_arc::<Inode>() {
            Ok(inode) => inode,
            Err(_) => panic!("Failed to downcast Arc<dyn ...> to Arc<Inode>"),
        };        
...
// where
        ...
        Self: Sized,
    {
        ...
        let parent_dir_specific = match parent_dir.clone().downcast_arc::<Inode>() {
            Ok(inode) => inode,
            Err(_) => panic!("Failed to downcast Arc to Inode"),
        };        
