...
impl Ext4OSInode {
    pub fn first_root_inode(ext4fs: &Arc<dyn VFS>) -> Arc<dyn File> {
        let ext4fs_concrete = match ext4fs.clone().downcast_arc::<Ext4FileSystem>() {
            Ok(fs) => fs,
            Err(_) => panic!("Failed to downcast Arc<dyn VFS> to Arc<Ext4FileSystem>"),
        };        
        // 先获取ROOT_INODE
...
impl File for Ext4OSInode {
    ...
    fn unlink(&self, delete: bool) -> Result<(), isize> {
        ...
        if inode_ref.inode.get_file_type() == DiskInodeType::Directory {
            ...
            let parent = match parent_osinode.clone().downcast_arc::<Ext4OSInode>() {
                Ok(inode) => inode,
                Err(_) => panic!("Failed to downcast Arc<dyn File> to Arc<Ext4OSInode>"),
            };            
